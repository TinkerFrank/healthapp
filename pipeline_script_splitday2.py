...
########################
### CLEANING for DF3 ###  CLIP outliers IQR method, BMI feature
######################## 
dftemp = df.copy()
...
########################
### CLEANING for DF4 ### DROP outliers IQR method, BMI feature
######################## 

Q1 = dftemp.quantile(0.25)
Q3 = dftemp.quantile(0.75)
IQR = Q3 - Q1
df4 = dftemp[~((dftemp < (Q1 - 1.5 * IQR)) |(dftemp > (Q3 + 1.5 * IQR))).any(axis=1)]

# save to SQLlite
df.to_sql('df4', if_exists='replace', con=dbConnection)
